...
- Added Python 3.13 to the CI test matrices, dropped Python 3.10 support, and restricted OpenGHG to Python 3.11-3.13 while the pinned Zarr/numcodecs combination lacks a Python 3.14 wheel. Also required `openghg-defs>=0.0.13` to remove its erroneous `pytest<=7.0.0` runtime dependency and adopted its updated resource API, and updated a conda-env `mypy` pin that had no Python 3.13 conda-forge build. [PR #1741](https://github.com/openghg/openghg/pull/1741), [Issue #1742](https://github.com/openghg/openghg/issues/1742)
