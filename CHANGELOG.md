...
- Added Python 3.13 and 3.14 to the CI test matrices and dropped Python 3.10 support, fixing an incompatible `scipy` upper bound (no Python 3.14 wheel), working around an erroneous `openghg-defs` transitive `pytest<=7.0.0` pin for uv project and release installs, and updating a conda-env `mypy` pin that had no Python 3.13/3.14 conda-forge build. [PR #1741](https://github.com/openghg/openghg/pull/1741), [Issue #1742](https://github.com/openghg/openghg/issues/1742)
