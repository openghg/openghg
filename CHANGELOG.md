...
- Added a lazy, Numba-enhanced `fp_x_flux_keep_space` analysis operator that preserves source and spatial dimensions, supports single-source, regular coarse-frequency flux, and irregular or non-monotonic footprint release times with explicit interval-start alignment, and includes optional atomic ppm Zarr persistence and worker warm-up helpers.
