...
- Linked file hashes to datasource UUIDs so deleting a datasource allows its source files to be re-imported without `force=True`, while surviving datasources from shared files are skipped. Also removed the implicit shift to `if_exists="new"` when `force=True` is used. [PR #1687](https://github.com/openghg/openghg/pull/1687)
