...
def _load_config() -> Dict:
    """Loads the OCI Config for accessing the object store
...
def get_bucket(bucket_name: str) -> Dict:
    ...
    try:
        ...
        raise ObjectStoreError(f"Error retrieving bucket {bucket_name}: {str(e)}")
...
def upload(
    bucket_name: str, key: str, data: Optional[bytes] = None, filepath: Optional[Union[str, Path]] = None
) -> Response:
    ...
    if data is not None and filepath is None:
        ...
        response = upload_manager.upload_stream(
            namespace_name=namespace, bucket_name=bucket_name, object_name=key, stream_ref=byte_stream
        )
...
def create_par(
    ...
    response = object_storage.create_preauthenticated_request(
        namespace_name=namespace, bucket_name=bucket_name, create_preauthenticated_request_details=request
    )
...
def delete_par(par_id: str, bucket_name: str) -> None:
