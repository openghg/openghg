...
import subprocess
import sys
from typing import List, IO, Union, Tuple, Literal
...
def check_if_need_new_version(
    if_exists: Literal["default", "new", "replace"] = "default",
    current_data: Literal["default", "keep", "ignore"] = "default",
) -> bool:
    """
    Check combination of if_exists and current_data keywords to determine
    whether a new version should be created.

    Output related to these parameters:
        - if_exists="default", current_data=None
        ...
        - if_exists="replace"/"new", current_data=None
           - new_version=True - If a scheme has been set for the
             combination of new and current data and current_data is None,
             create a new version.
        - if_exists="default"/"replace"/"new", current_data=True
           - new_version=True - If current_data is explicitly set
             to True, create a new version.
        - if_exists="default"/"replace"/"new", current_data=False
           - new_version=False - If current_data is explicitly set
             to False, allow previous version to be overwritten.

    Args:
        if_exists: How to combine new and current data, if present.
        current_data: Whether to save current data or replace this.
    Returns:
        bool: Whether new version should be created
    """
    # Determining whether a new version should be created based on inputs.
    if if_exists == "default" and current_data == "default":
        # Add new (non-overlapping) data on the same version
        new_version = False
    elif if_exists != "default" and current_data == "default":
        # If data could be modified based on if_exists input
        # default to creating a new version.
        new_version = True
    elif current_data == "ignore":
        # Otherwise match new version to the current_data input.
        new_version = False
    elif current_data == "keep":
        new_version = True
    else:
        raise ValueError("Invalid combination of if_exists and current_data. See documentation.")
