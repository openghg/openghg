...
def parse_cams(
    ...
    with xr_open_fn(filepath_p).chunk(chunks) as ds:
        ...
        ds["species"].attrs["units"] = unit_converter.get(ds["species"].attrs["units"],ds["species"].attrs["units"])
        ...
        bc_data.attrs["files_used"] = (
            ", ".join([file.name for file in filepath_p]) if isinstance(filepath_p,list) else filepath_p.name
        )
...
def interp1d_np(data: np.ndarray, x: np.ndarray, xi: np.ndarray, **kwargs) -> np.ndarray:
    return np.interp(xi, x, data, **kwargs)

def xr_interp(data: xr.DataArray, dim: str, interp_vals: np.ndarray, coord: str | None = None) -> xr.DataArray:
