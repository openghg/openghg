...
def _preprocess(ds: xr.Dataset, quality_filter: bool) -> xr.Dataset:
    ...
    if not quality_filter:
        logger.warning(
            "It is preferred to use the quality filter (qual_flag == 1). Returning unfiltered data."
        )
    else:
        ...
        if ds.time.size == 0:
            raise ValueError(
                "Dataset is empty after filtering. Nothing to store. Possibly all the data have qual_flag != 1."
            )
