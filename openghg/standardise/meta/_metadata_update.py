...
def create_surface_metadata(user_inputs: Dict[str, Any],
                            stored_data: Dict[str, Any],
                            attributes: Dict[str, Any],
                            metadata_needed: List = metadata_default_keys(),
                            allow_missing: bool = False,
                            ) -> Dict[str, Any]:
    """
    """

    attribute_sources = [user_inputs, stored_data, attributes]

    metadata = {}
    missing_metadata = []
    # Search parameter sources (in order) and populate metadata
    for param in metadata_needed:
        for source in attribute_sources:
            ...
            if allow_missing:
                logger.warning(f"Unable to add '{param}' key to metadata")
            else:
                missing_metadata.append(param)
    
    if missing_metadata:
        raise ValueError(
            "Cannot extract or infer the following parameters needed for metadata from user inputs, stored data or attributes:\n"
