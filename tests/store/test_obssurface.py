...
def test_read_CRDS(bucket):
    ...
    with xr.open_mfdataset(memory_store, engine="zarr", combine="by_coords", consolidated=False) as ch4_data:
...
def test_read_openghg_format(bucket):
    ...
    with xr.open_mfdataset(co2_data.memory_store(), engine="zarr", combine="by_coords", consolidated=False) as co2_data:
...
def test_read_noaa_raw(bucket):
    ...
    with xr.open_mfdataset(co_datasource.memory_store(), engine="zarr", combine="by_coords", consolidated=False) as co_data:
...
def test_read_noaa_metastorepack(bucket):
    ...
    with xr.open_mfdataset(paths=memory_store, engine="zarr", combine="by_coords", consolidated=False) as ch4_data:
