...
def standardise():
    """Use `standardise(**kwargs)` to standardise surface data.

    We only deal with surface data in this test module, and most
    ...
    return partial(standardiser.standardise, data_type="surface")
...
def test_read_CRDS():
    ...
    filepath = get_surface_datapath(filename="bsd.picarro.1minute.248m.min.dat", source_format="CRDS")
    results = standardiser.standardise("surface", filepath=filepath, source_format="CRDS", site="bsd", network="DECC")
