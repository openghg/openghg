...
def read_data_refactor(bucket, binary_data, metadata, file_metadata):
    return Emissions(bucket).read_data(binary_data=binary_data, metadata=metadata, file_metadata=file_metadata)


def transform_data_refactor(bucket, datapath, database, **kwargs):
    return Emissions(bucket).transform_data(datapath=datapath, database=database, **kwargs)
...
def standardise(bucket):
    ...
    return partial(standardiser.standardise, data_type="emissions")
...
class TestCardamom:
    ...
    def test_read_binary_data(self, mocker, bucket):
        ...
        results = read_data_refactor(bucket, binary_data=binary_data, metadata=metadata, file_metadata=file_metadata)
...
class TestEDGAR:
    ...
    def load_edgar(self, standardise):
        def _load_edgar(version, year, database_version: bool = True):
            ...
            #_standardise = get_emissions_standardiser(bucket)
    ...
    def test_add_edgar_database(self, bucket):
        ...
        proc_results = transform_data_refactor(bucket, datapath=test_datapath, database=database, date=date)
    ...
    def test_transform_and_add_edgar_database(self):
        ...
        bucket = get_bucket()
        proc_results = transform_data_refactor(bucket, datapath=test_datapath, database=database, date=date, domain=domain)
