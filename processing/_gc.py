...
def gc_precisions_read(precisions_file):
    ...
    # Drop duplicates
    precision = precision.reset_index().drop_duplicates(
        ...
        for inleti, inlet in enumerate(inlets):
            ...
            if (inlet == "any") or (inlet == "air"):
                ...
                if inlet[0:4] == "date":
                    ...
                    select_inlet = [fnmatch.fnmatch(
                        i, inlet) for i in ds.Inlet.values]
            ...
            Inlets = set(ds_sp.where(
                ds_sp[sp + " status_flag"] == 0, drop=True).Inlet.values)
