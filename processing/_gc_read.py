def gc_data_read(dotC_file, scale={}, units={}):
    ...
    for i in range(len(df)):
        # Create a load of datetime objects from these five columns
        time.append(_datetime.datetime(df.yyyy[i], df.mm[i], df.dd[i], df.hh[i], df.mi[i]))
        # Read analysis time  - check datafile for analysis time
        if "ryyy" in list(df.keys()):
            time_analysis.append(_datetime.datetime(df.ryyy[i], df.rm[i], df.rd[i], df.rh[i], df.ri[i]))
