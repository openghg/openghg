...
def attributes(ds, species, site,
    ...
    try:
      ...
      print("ERROR: Can't find species %s in column names %s" %
            (species, list(ds.keys())))
    ...
    if (species.upper()[0] == "D" and species.upper() != "DESFLURANE") or species.upper() == "APO":
        ...
        sp_long = "mole_fraction_of_" + \
            species_translator[species.upper()][1] + "_in_air"
    else:
    ...
    print(f"Ancilliary attributes : {species_out}")
    ...
    if len(flag_key) > 0:
        ...
        ds[flag_key].attrs = {"flag_meaning":
                              "0 = area, 1 = height",
                              "standard_name":
                              ds[species_out].attrs["long_name"] +
                                  " integration_flag",
                              "comment":
